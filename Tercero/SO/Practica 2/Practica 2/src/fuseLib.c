...
static int my_unlink(const char *path){
	...
	//Comprobamos si la longitud del nombre es valida:
	if(strlen(path +1) > myFileSystem.superBlock.maxLenFileName){
		return -ENAMETOOLONG;
	}

	int i; 				//i auxiliar para bloces
	int idxDir;			//indice del fichero en el directorio array files
	int i_Nodo_File;	//indice del nodo-i del fichero direcroty.files[idxDir].node.Idx
	int bloque_act;		//Bloque auxiliar

	char bloque[BLOCK_SIZE_BYTES];	//Bloque auxiliar para usar como buffer vacio en el writeBlock(...)

	//Ponemos all el bloque a ceros para borrar la info de los bloques del fichero
	memset(bloque,0, sizeof(char)*BLOCK_SIZE_BYTES);

	//Este mensaje se muestra siempre
	fprintf(stderr, "--->>>my_unlink: path %s\n", path);

	//Obtenemos el numero i del fichero (en el array de archivos)
	if((idxDir = findFileByName(&myFileSystem, (char *)path + 1)) == -1) {
	        return -ENOENT;
	}

	//Nodo i del fichero
	i_Nodo_File = myFileSystem.directory.files[idxDir].nodeIdx;

	//Comprobamos que sea un fichero regular:
	/*if(myFileSystem.nodes[i_Nodo_File]->type != 0){
		return -EISDIR;
	}*/

	//se podria usar:
	/*if(resizeNode(idxDir,0)<0){
		return -EIO;
	}*/

	//Borramos los bloques del fichero en el mapa de bits
	for(i=0; i < myFileSystem.nodes[i_Nodo_File]->numBlocks; i++){

			myFileSystem.superBlock.numOfFreeBlocks++;
			//Buscamos el bloque actual:
			bloque_act = myFileSystem.nodes[i_Nodo_File]->blocks[i];
			//Lo ponemos a 0 en el bit map
			myFileSystem.bitMap[bloque_act] =0;


			//Llamamos a writeblock en el bloque actual, para que copie un bloque vacio
			/*if(writeBlock(&myFileSystem, bloque_act, &bloque)){
				return -EIO;
			}*/

			/*if(lseek(myFileSystem.fdVirtualDisk, bloque* BLOCK_SIZE_BYTES, SEEK_SET) == (off_t -1) ||
					(write(myFileSystem.fdVirtualDisk, &bloque, BLOCK_SIZE_BYTES) == -1)){
					perror("error al buscar el file Descriptor en unlink | Al borrar la info del bloque: %d\n", i);
					return -EIO;
			}*/
	}
	...
	//Actualizamos el directorio:
		...
		myFileSystem.directory.files[idxDir].freeFile =1;	//Marcar el booleano de libre asociado a esa posicion en el directorio a 1
		memset(myFileSystem.directory.files[idxDir].fileName, '\0', sizeof(char)* (MAX_LEN_FILE_NAME+1));	//Borrar el nombre del archivo (ponerlo a 0x00..)
		myFileSystem.directory.numFiles--;	//Actualizamos el contador de archivos

		//Actualizamos el directorio
		updateDirectory(&myFileSystem);

	//Actualizamos nodos-i:

		//Liberar el nodo-i del fichero y guardamos en disco
		myFileSystem.nodes[i_Nodo_File]->freeNode = 1;
		updateNode(&myFileSystem, i_Nodo_File, myFileSystem.nodes[i_Nodo_File]);

		//Liberamos el espacio en memorio del nodo-i del fichero y lo apuntamos a NULL
		free(myFileSystem.nodes[i_Nodo_File]);
		myFileSystem.nodes[i_Nodo_File] = NULL;

		//Actualizamos el contador de i-nodos libres
		myFileSystem.numFreeNodes++;
...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi){
	...
	if (offset < size) {
		...
		if (offset + size > bytes2read)
			size = bytes2read - offset;
		...
		while (bytes2read){
	    	...
	    	//Movemos el fd y leemos un bloque , verificando que no haya errores
	 		if((lseek(myFileSystem.fdVirtualDisk, currentBlock * BLOCK_SIZE_BYTES, SEEK_SET) == (off_t) - 1) ||
	 			...
	 			perror("Fallo lseek/read en my_read");
	 			return -EIO;
	 		}
			...
			//esto ya esta leido
	 	    //bytes que nos quedan por leer
			bytes2read -= (i-offBlock);

			//Dejo el offset correctamente situado, si he leido hasta el final del bloque lo dejo a 0.
			offset+=i;
    }
	} else
		size = 0;
...
struct fuse_operations myFS_operations = {
    ...
    .read 		= my_read,						// Read n bytes from a file 
    .unlink 	= my_unlink 					// Delete an existing file from the myFileSystem
};
